# =============================================================================
#  dd-java-weblogic — Verify Script
#  Validates SSI injection into wlsvc.exe (WebLogic Windows service wrapper).
#  The ddinjector java.c detects wlsvc.exe via is_weblogic_service() and injects
#  the Java tracer into the in-process JVM.
#
...
try {
    $svc     = Get-Service -Name "WlsvcDemoSvc" -ErrorAction Stop
    $svcPass = ($svc.Status -eq "Running")
} catch { $svcPass = $false }
$results.checks["service_running"] = @{ service = "WlsvcDemoSvc"; pass = $svcPass }
if ($svcPass) { Write-Ok "WlsvcDemoSvc RUNNING" } else { Write-Fail "WlsvcDemoSvc NOT running" }
...
for ($i = 1; $i -le 10; $i++) {
    try {
        $r = Invoke-WebRequest -Uri "http://${TargetHost}:8090/health" -UseBasicParsing -TimeoutSec $TimeoutSec
        if ($r.StatusCode -eq 200) {
            $body = $r.Content | ConvertFrom-Json
            $healthPass = ($body.status -eq "ok") -and ($body.process -eq "wlsvc.exe")
            break
        }
    } catch {}
    if ($i -lt 10) { Start-Sleep -Seconds 3 }
}
$results.checks["health_8090"] = @{ uri = "http://${TargetHost}:8090/health"; pass = $healthPass }
if ($healthPass) { Write-Ok "Health endpoint OK on port 8090 (process=wlsvc.exe)" }
else             { Write-Fail "Health endpoint NOT responding on port 8090" }

# -- 3. DLL injection — ddinjector_x64.dll in wlsvc.exe ----------------------
Write-Section "DLL INJECTION CHECK (wlsvc.exe — WebLogic service wrapper path in java.c)"
$dllPass = Test-DllInjected -ProcessName "wlsvc.exe"
$results.checks["dll_injection_wlsvc"] = @{
    process = "wlsvc.exe"
    dll     = "ddinjector_x64.dll"
    pass    = $dllPass
    note    = "Detected by is_weblogic_service() in java.c (matches wlsvc.exe, wlsvcX64.exe)"
}
if ($dllPass) { Write-Ok "ddinjector_x64.dll loaded in wlsvc.exe (WebLogic injection path confirmed)" }
else          { Write-Fail "ddinjector_x64.dll NOT found in wlsvc.exe — WebLogic SSI injection failed" }
...
if (-not $DDApiKey) {
    ...
    for ($i = 1; $i -le 3; $i++) {
        try { Invoke-WebRequest -Uri "http://${TargetHost}:8090/ping" -UseBasicParsing -TimeoutSec 5 | Out-Null } catch {}
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        try {
            ...
            $q      = [Uri]::EscapeDataString("service:java-weblogic-app")
    ...
    $results.checks["traces_received"] = @{ service = "java-weblogic-app"; pass = $tracePass }
}
